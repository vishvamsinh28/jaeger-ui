...
jest.mock('./HopsSelector', () => {
  return function HopsSelector({ distanceToPathElems, handleClick, visEncoding }) {
    return (
      <div 
...
jest.mock('./LayoutSettings', () => {
  return function LayoutSettings({ density, setDensity, showOperations, toggleShowOperations }) {
    return (
      <div 
        data-testid="layout-settings"
        data-density={density}
        data-show-operations={showOperations}
      />
    );
  };
});
...
jest.mock('../../common/NameSelector', () => {
  return function NameSelector({ label, value, setValue, options, clearValue, ...props }) {
    ...
    return (
      <div data-testid={`name-selector-${label.toLowerCase()}`}>
        ...
        <select
          ...
          data-testid={`${label.toLowerCase()}-selector`}
        >
        ...
        {clearValue && (
          <button onClick={clearValue} data-testid={`clear-${label.toLowerCase()}`}>
            Clear
          </button>
        )}
      </div>
...
jest.mock('../../common/UiFindInput', () => {
  return function UiFindInput({ forwardedRef, inputProps, ...props }) {
    return (
      <input
        ref={forwardedRef}
...
describe('<Header>', () => {
  ...
  it('renders the operation selector when a service is selected', () => {
    ...
    rerender(<Header {...minProps} service={service} services={services} operation={operation} operations={operations} />);
